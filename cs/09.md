...
Stránky pro routy `Admin_Articles` a `Admin_ArticleDetail` budeme vytvářet až v další kapitole.
...
V business vrstvě máme třídu `UserService`, která má na starosti přihlašování uživatelů. Její metodě `SignIn` předáme objekt s uživatelským jménem a heslem a pokud je uživatel ověřen, tato metoda vrátí objekt `ClaimsIdentity` s informacemi o uživateli. V opačném případě vyhodí výjimku.
